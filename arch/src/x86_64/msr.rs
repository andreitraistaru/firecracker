...
impl MsrRange {
    ...
    fn contains(&self, msr: u32) -> bool {
        self.base <= msr && msr < self.base + self.nmsrs as u32
    }
}
...
const APIC_MSR_INDEXES: usize = 0x400;
...
fn create_boot_msr_entries() -> Vec<kvm_msr_entry> {
    let msr_entry = |msr| kvm_msr_entry {
        index: msr,
        data: 0x0,
        ..Default::default()
    };
    ...
    vec![
        msr_entry(MSR_IA32_SYSENTER_CS),
        msr_entry(MSR_IA32_SYSENTER_ESP),
        msr_entry(MSR_IA32_SYSENTER_EIP),
        // x86_64 specific msrs, we only run on x86_64 not x86.
        msr_entry(MSR_STAR),
        msr_entry(MSR_CSTAR),
        msr_entry(MSR_KERNEL_GS_BASE),
        msr_entry(MSR_SYSCALL_MASK),
        msr_entry(MSR_LSTAR),
        // end of x86_64 specific code
        msr_entry(MSR_IA32_TSC),
        kvm_msr_entry {
...
mod tests {
    ...
    fn test_msr_whitelist() {
        for range in WHITELISTED_MSR_RANGES.iter() {
            for msr in range.base..(range.base + range.nmsrs as u32) {
