...
use crate::virtio::{
    GenericVirtioDeviceState, SpecificVirtioDeviceStateError, VIRTIO_MMIO_INT_CONFIG,
...
const INFLATE_QUEUE_IDX: usize = 0;
const DEFLATE_QUEUE_IDX: usize = 1;
...
impl BalloonEpollHandler {
    ...
    fn process_inflate(&mut self) -> Result<(), DeviceError> {
        ...
        // If an interrupt is needed.
        let mut needs_interrupt = false;

        // Repeatedly remove descriptors from the inflation queue
        // until there are none left.
        while let Some(avail_desc) = self.queues[INFLATE_QUEUE_IDX].pop(&self.mem) {
            ...
            // it contains only SIZE_OF_U32 byte addresses).
            if !avail_desc.is_write_only() && desc_len % SIZE_OF_U32 as u32 == 0 {
                // Now, for all addresses in the descriptor, try to
                // remove the corresponding pages from the guest.
                for index in (0..desc_len).step_by(SIZE_OF_U32) {
            ...
            // Acknowledge receipt of the descriptor.
            needs_interrupt = true;
            self.queues[INFLATE_QUEUE_IDX].add_used(&self.mem, avail_desc.index, 0);
        }

        // Inform guest that used queue is filled.
        if needs_interrupt {
        ...
        for (page_frame_number, range_len) in page_ranges {
            ...
            // Finally, actually remove (or advise the kernel that we don't need) the page from memory.
            self.mem
                .madvise_range(
                    guest_address,
                ...
                .or_else(|err| match err {
                    memory_model::GuestMemoryError::MemoryAccess(
                        _,
                        memory_model::MemoryMappingError::SystemCallFailed(ref inner_error),
                    ) if inner_error.raw_os_error() == Some(libc::EINVAL)
                        || inner_error.raw_os_error() == Some(libc::EACCES) =>
                    {
                        self.mem.madvise_range(
                            guest_address,
                            u64::from(range_len) << VIRTIO_BALLOON_PFN_SHIFT,
                            libc::MADV_DONTNEED,
                        )
    ...
    fn process_deflate(&mut self) -> Result<(), DeviceError> {
        // Read event.
        self.queue_evts[DEFLATE_QUEUE_IDX].read().map_err(|e| {
            error!("failed reading deflate queue EventFd: {}", e);
            METRICS.balloon.event_fails.inc();
            DeviceError::FailedReadingQueue {
                event_type: "deflate queue event",
                underlying: e,
            }
        })?;
        ...
        // until there are none left.
        while let Some(avail_desc) = self.queues[DEFLATE_QUEUE_IDX].pop(&self.mem) {
            // Acknowledge receipt of the descriptor.
            self.queues[DEFLATE_QUEUE_IDX].add_used(&self.mem, avail_desc.index, 0);

            // Eventually, the guest must receive an interrupt
            // to be able to react to this acknowledgement.
            needs_interrupt = true;
        }
        if needs_interrupt {
            self.signal_used_queue();
...
impl Balloon {
    ...
    pub fn new(
        ...
        Ok(Balloon {
            ...
            actual_pages: Arc::new(AtomicUsize::new(actual_pages)),
...
impl VirtioDevice for Balloon {
    ...
    fn config_space(&self) -> Vec<u8> {
        ...
        // so unwrap is fine.
        (&mut ret[0..])
            .write_u32::<LittleEndian>(self.num_pages.load(Ordering::SeqCst) as u32)
            .unwrap();
        (&mut ret[SIZE_OF_U32..])
    ...
    fn write_config(&mut self, offset: u64, mut data: &[u8]) {
        ...
        if offset != SIZE_OF_U32 as u64 || data.len() != SIZE_OF_U32 {
            return;
        }
        // This read can't fail as it fits in the declared array so unwrap is fine.
        let new_actual = data.read_u32::<LittleEndian>().unwrap();
        self.actual_pages
    ...
    fn activate(
        ...
        let inflate_rawfd = handler.queue_evts[0].as_raw_fd();
        let deflate_rawfd = handler.queue_evts[1].as_raw_fd();
...
impl BalloonState {
    ...
    pub fn restore(
        ...
        // The following reads cannot fail as they fit in the size of the config space.
        let balloon = Balloon::new(
            (&config_space[0..]).read_u32::<LittleEndian>().unwrap() as usize,
            (&config_space[SIZE_OF_U32..])
                .read_u32::<LittleEndian>()
                .unwrap() as usize,
            (avail_features & (1 as u64) << VIRTIO_BALLOON_F_MUST_TELL_HOST) != 0,
            (avail_features & (1 as u64) << VIRTIO_BALLOON_F_DEFLATE_ON_OOM) != 0,
            epoll_config,
        );
...
mod tests {
    ...
    use memory_model::AnonMemoryDesc;
    ...
    macro_rules! check_metric_after_block {
        ($metric:expr, $delta:expr, $block:expr) => {{
            ...
            let _ = $block;
            let after = $metric.count();
            assert_eq!(after, before + $delta, "unexpected metric value");
        }};
    }
    ...
    impl DummyBalloon {
        fn new(num_pages: usize, must_tell_host: bool, deflate_on_oom: bool) -> Self {
            ...
            DummyBalloon {
                balloon: Balloon::new(num_pages, 0, deflate_on_oom, must_tell_host, epoll_config)
                    .ok()
                    .unwrap(),
                epoll_raw_fd,
                _receiver,
    ...
    impl Balloon {
        ...
        fn set_actual_pages(&self, new_actual_pages: usize) {
            self.actual_pages.store(new_actual_pages, Ordering::Relaxed);
        }
    }
    ...
    fn make_test_balloonepollhandler(
        ...
        let queues = vec![inflate_queue, deflate_queue];
        let interrupt_status = Arc::new(AtomicUsize::new(0));
        let interrupt_evt = EventFd::new().unwrap();
        let queue_evts = vec![EventFd::new().unwrap(), EventFd::new().unwrap()];
    ...
    fn test_handle_inflate_event() {
        // First, test inflating an anonoymous shared mapping.
        //
        // The testing methodology will be to dirty a page of
        // anonymous shared memory, send an inflate request for that
        // page, and then check that the ballooned memory page is
        // set to 0 (since this is a checkable side effect of using
        ...
        check_metric_after_block!(METRICS.balloon.event_fails, 1, {
            ...
            // But don't trigger `h.queue_evts[0]`.
        ...
        // Now, test inflating a private, file-backed mapping.
        //
        // The testing methodology will be to dirty a private file-backed page,
        // send an inflate request for that page, and then
        // check that the ballooned memory page contains only zeroes
        // This will prove that the special case for file-backed pages,
        // in which, rather than using `madvise` with `remove`, we use
        // `madvise` with `dontneed` is entered, and works succesfully.

        // Create a file to back our memory mapping.
        let file = tempfile().unwrap();
        assert_match!(file.set_len(0x5000), Ok(_));
        let ranges = [memory_model::FileMemoryDesc {
            gpa: GuestAddress(0),
            size: 0x5000,
            fd: file.as_raw_fd(),
            offset: 0,
        ...
        check_metric_after_block!(METRICS.balloon.inflate_count, 1, {
            ...
            infq.dtable[0].set(0x10, 4, VIRTQ_DESC_F_NEXT, 1);
            h.queue_evts[0].write(1).unwrap();
            let tmp = h.handle_event(BALLOON_INFLATE_EVENT, epoll::Events::EPOLLIN);
            ...
            // Check that the range of memory written contains only 0's.
            for i in 0..0x1000 {
                assert_match!(
                    mem.read_obj_from_addr::<u8>(GuestAddress((2 << 12) + i)),
                    Ok(0)
    ...
    fn test_handle_deflate_event() {
        ...
        // Test an event that forgets to trigger `queue_evts[1]`.
        check_metric_after_block!(METRICS.balloon.event_fails, 1, {
            ...
            // Forget to trigger `queue_evts[1]`.
            // Now check that we get an appropriate error.
            assert_match!(
    ...
    fn test_ack_features() {
        ...
        for deflate_on_oom in vec![true, false].iter() {
            for must_tell_host in vec![true, false].iter() {
                ...
                assert_eq!(b.acked_features(), 0 as u64);
    ...
    fn test_config_space() {
        let mut dummy = DummyBalloon::new(0x8765_4321, false, false);
        dummy.balloon().set_actual_pages(0x1234_5678);
        let b = dummy.balloon();

        assert_eq!(
            b.config_space(),
            vec![0x21, 0x43, 0x65, 0x87, 0x78, 0x56, 0x34, 0x12]
        )
    }
    ...
    fn test_balloon_state() {
        let epoll_raw_fd = epoll::create(true).unwrap();
        let (sender1, _receiver) = mpsc::channel();
        let (sender2, _receiver) = mpsc::channel();
        let epoll_config1 = EpollConfig::new(0, epoll_raw_fd, sender1);
        let epoll_config2 = EpollConfig::new(0, epoll_raw_fd, sender2);

        let b = Balloon::new(321, 123, true, false, epoll_config1).unwrap();

        let generic_state = GenericVirtioDeviceState::new(
            TYPE_BALLOON,
            "balloon",
            b.avail_features(),
            b.acked_features(),
        ...
        let specific_state = BalloonState::new(&b, None).unwrap();

        let b2 = specific_state
            .restore(&generic_state, epoll_config2)
            .unwrap();

        assert_eq!(b.avail_features(), b2.avail_features());
        assert_eq!(b.acked_features(), b2.acked_features());
        assert_eq!(b.config_space(), b2.config_space());
    }
}
