...
mod tests {
    ...
    fn test_virtio_device() {
        ...
        assert_eq!(ctx.device.avail_features(), ctx.device.avail_features);
        assert_eq!(ctx.device.acked_features(), ctx.device.acked_features);

        // The config space should only contain the cid stored as an le64 value.
        assert_eq!(
            ctx.device.config_space().as_slice(),
            &ctx.device.cid.to_le_bytes()[..]
        );
        assert_eq!(ctx.device.config_space().len(), 8);
