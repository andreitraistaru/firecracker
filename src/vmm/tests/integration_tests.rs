...
use vmm::utilities::test_utils::{create_vmm, default_vmm, run_events_to_completion};
...
fn test_build_microvm() {
    ...
    // Success case, also tests exiting vCPUs works.
    let (vmm, event_manager) = default_vmm(None);
    vmm.lock().unwrap().stop();
    let exit_code = run_events_to_completion(event_manager);
    assert_eq!(exit_code, FC_EXIT_CODE_OK);
}
...
fn test_dirty_bitmap_success() {
    ...
    let (vmm, event_manager) = dirty_tracking_vmm(Some(NOISY_KERNEL_IMAGE));
...
fn test_disallow_snapshots_without_pausing() {
    let (vmm, event_manager) = default_vmm(Some(NOISY_KERNEL_IMAGE));
...
fn verify_create_snapshot(is_diff: bool) -> (TempFile, TempFile) {
    ...
    let (vmm, evmgr) = create_vmm(Some(NOISY_KERNEL_IMAGE), is_diff);
    ...
    let exit_code = run_events_to_completion(evmgr);
    assert_eq!(exit_code, FC_EXIT_CODE_OK);
