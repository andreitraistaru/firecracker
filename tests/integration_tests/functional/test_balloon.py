...
# pep8: disable=E501
def test_pause_balloon(test_microvm_with_ssh_and_balloon, network_config):
    """Verify that inflating, deflating, pausing, resuming works."""
    test_microvm = test_microvm_with_ssh_and_balloon
    test_microvm.spawn()
    test_microvm.basic_config()
    _tap, _, _ = test_microvm.ssh_network_config(network_config, '1')
    ...
    # Now quickly pause, possibly before the inflate is done.
    ...
    # Now quickly pause again
    response = test_microvm.actions.put(action_type='PauseVCPUs')
    assert test_microvm.api_session.is_status_no_content(response.status_code)

    # And now resume again.
    response = test_microvm.actions.put(action_type='ResumeVCPUs')
    assert test_microvm.api_session.is_status_no_content(response.status_code)
    time.sleep(5)
    fourth_reading = get_rss_mem_by_pid(firecracker_pid)
